...
namespace emujv2Api.Controller
{
    ...
    public class WebApiController : ControllerBase
    {
        ...
        [HttpPost]
        public string UpdateValidator([FromBody] R1FormCons data)
        {
            TokenFunc Token = new TokenFunc();
            PublicCons RetDat = new PublicCons();
            string conn = _config.GetValue<string>("KTMBParam:DbConnection");
            ...
            {
                // FIX: Passed both the verifier and the target report code
                Salah = ret.UpdateValidator(data.ValidatedBy, data.RptCode);
        ...
        public string GetAttdList(string RptCode)
        {
            ...
            if (!string.IsNullOrEmpty(User.Userid))
            {
                return ret.GetAttdList(RptCode);
            }
            else
            {
                RetDat.status = "99";
